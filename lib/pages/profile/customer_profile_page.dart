...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> toggleBiometric(bool value) async {
    ...
    if (value) {
      ...
      await AuthService().updateBiometricStatus(
        userId: userId ?? 0,
        biometricEnabled: true,
      );
      await SecureStorageHelper.saveBiometricEnabled(
        enabled: true,
        userId: userId ?? 0,
        role: role ?? 2,
      );
      await SecureStorageHelper.saveBiometricUserData(
        token: token ?? "",
        userId: userId ?? 0,
  ...
  Future<void> _loadUser() async {
    ...
    if (userId != 0) {
      ...
      if (customerId != null) {
        try {
          final voucherRes = await promotionService.getCustomerVouchers();
          if (voucherRes["success"] == true) {
            final List<dynamic> vouchers = voucherRes["data"];
            voucherCount = vouchers
                .where(
                  (v) =>
                      v['customer_id'].toString() == customerId &&
                      v['status']?.toString().toUpperCase() != 'USED' &&
                      v['status']?.toString().toUpperCase() != 'CANCELLED',
                )
                .length;
          }
        } catch (_) {}
        ...
        try {
          final reviewRes = await reviewService.getAllReviews();
          if (reviewRes["success"] == true) {
            final List<dynamic> allReviews = reviewRes["data"];
            reviewCount = allReviews
                .where(
                  (r) =>
                      r["customer_id"].toString() == customerId &&
                      r["is_delete"] != true,
                )
                .length;
          }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: RefreshIndicator(
          onRefresh: _loadUser,
          color: const Color(0xFF1A7A6D), // tealColor
                ...
                _buildActionCard(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const CustomerMyPromoPage(),
                      ),
                    );
                  },
                ),
                ...
                _buildActionCard(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (_) => const CustomerMyReviewsPage(),
                      ),
                    );
                  },
                ),
      ...
      children: [
        Container(
          ...
          child: Row(
            ...
            children: [
              Icon(icon, size: 24, color: const Color(0xFF1A1A2E)),
